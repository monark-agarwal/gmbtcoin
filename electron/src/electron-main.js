...
function startSkycoin() {
  ...
  if (skycoin) {
    console.log('solarbankerscoinalready running');
  ...
  var exe = (() => {
    switch (process.platform) {
      case 'darwin':
        return path.join(appPath, '../../Resources/app/iytecoin');
      case 'win32':
        // Use only the relative path on windows due to short path length
        // limits
        return './resources/app/iytecoin.exe';
      case 'linux':
        return path.join(path.dirname(appPath), './resources/app/iytecoin');
      default:
        return './resources/app/iytecoin';
    }
  })()
  ...
  skycoin.on('error', (e) => {
    dialog.showErrorBox('Failed to start iytecoin', e.toString());
    app.quit();
  });
  ...
  skycoin.on('close', (code) => {
    // log.info('solarbankerscoinclosed');
    console.log('solarbankerscoinclosed');
    reset();
  });

  skycoin.on('exit', (code) => {
    // log.info(solarbankerscoinexited');
    console.log('solarbankerscoinexited');
    reset();
  });
}
...
function createWindow(url) {
  ...
  ses.clearCache(function () {
    console.log('Cleared the caching of the iytecoin wallet.');
  });
